...
HEAD: 9f521ae9fd032fabf44d4b1dd0431afe3daeb105
...
| `check` | `uvx --from rust-just just check` | 0 | On the post-#116 code/baseline HEAD, Ruff, strict mypy over 181 files, Vulture, and 1,209 tests passed; one Linux-only mutation self-test skipped on Windows. |
...
| `invariants` | `uvx --from rust-just just check-invariants` | 0 | 325 critical tests passed, including constant-basis windows, chronological H4 drawdown, gross/swap decomposition, signal parity, and direction integration. |
| `mutation-on-touched-critical` | GitHub Actions `Critical mutation`, run `30342712156` | 0 | Linux/Python 3.13 measured 4,646 mutants: 4,236 killed, 410 exact-name survivors, and zero no-tests, suspicious, timeout, unchecked, interrupted, segfault, or type-check outcomes. The retained report compares with zero issues against the regenerated baseline. |
...
| `security` | `uvx --from rust-just just check-security` | 0 | Post-#116 secret scan clean; pip-audit reports no known vulnerabilities; Ruff security checks pass. |
...
| `post-116-rebase` | `git rebase origin/main` plus `git range-diff`/scope audit | 0 | Rebased onto `main@71a6acc`; the ten new behavioural cases remain test-only and issue #99 production behavior is unchanged. The independent-review caveat remains in `review.md`. |
| `mutation-report-replay` | retained run `30342712156` report through `MutationReport`, `load_baseline`, and `check_baseline` | 0 | The committed baseline names all 410 measured survivors exactly and contains all 23 targets, including `trade-direction-extraction`; comparator issues are `[]`. |
...
fact-sheet fixture now supplies the categorical field explicitly. The earlier focused impact run
passes 418 tests; the post-#116 full suite, including the mutation-strength additions, passes 1,209
tests. No Stage-3/4 artifact was regenerated because
...
The Actions blocker is resolved. Pre-test Linux run `30339335275` measured 24 branch-specific
survivors: ten in `timed_trades_from_report` and fourteen in `_synchronized_h4_minima`. The new
behavioural tests killed 17 of those 24:

- trade-direction extraction: `mutmut_5`, `_9`, `_11`, `_14`, `_15`, `_16`, `_28`, `_35`, `_36`;
- synchronized-H4 replay: `mutmut_92`, `_120`, `_172`, `_173`, `_201`, `_257`, `_314`, `_315`.

The same tests also killed the pre-existing baseline survivor
`research.portfolio.sizing.x__synchronized_h4_minima__mutmut_91`. The seven remaining
branch-specific survivors are equivalent in principle and are classified separately, not under a
shared convenience waiver:

- `timed_trades_from_report__mutmut_27`: authoritative Nautilus `Money` strings contain no digit
  separator, and Python `float()` accepts underscores natively, so the replacement cannot alter the
  parsed value. The consistent four-site cleanup is issue #120.
- `_synchronized_h4_minima__mutmut_124`: the internal `observed` array is assigned only `True`,
  consumed only by scalar truth tests, and never returned; default float64 `0.0/1.0` and bool
  `False/True` are observationally identical there.
- `_synchronized_h4_minima__mutmut_139`: `np.zeros` already defaults to float64, so removing the
  explicit float64 dtype changes neither bytes nor values.
- `_synchronized_h4_minima__mutmut_152`: `Decimal("1e-12")` and `Decimal("1E-12")` are the same
  Decimal value and exponent.
- `_synchronized_h4_minima__mutmut_253`: the branch can execute only on the last sorted timestamp;
  `continue` and `break` therefore both leave the loop without any intervening observable action.
- `_synchronized_h4_minima__mutmut_258`: the next element of a sorted set is strictly greater than
  the current element, so `<` and `<=` are both false for this comparison.
- `_synchronized_h4_minima__mutmut_318`: the private function's valid `FloatArray` inputs make
  `np.minimum` float64 already, and later writes are floats; the final dtype argument cannot convert
  anything.

After rebasing onto merged PR #116, run `30341283390` measured `4,236 / 4,646` killed and `410`
survivors. It failed only because the still-committed `4,568 / 417` ratchet was stale. The baseline
was then wholesale-regenerated from that retained report: it adds `trade-direction-extraction`,
removes every now-killed exact name, and records the seven individual equivalence reasons. Final
Linux run `30342712156` reproduced `4,236 / 4,646` killed and all `410` exact-name survivors, with
zero unhealthy statuses, and passed the regenerated ratchet.

## Coverage and mutation

The rebased code HEAD has 1,209 passing tests, 418 earlier focused impact tests, 253 earlier explicit
integration tests, 325 critical invariant tests, and twenty-one properties passing twice. New
behavioural coverage spans the producer's BUY/SELL/fail-closed boundary, skipped/pre-roll report
rows, UTC and window-boundary attribution, authoritative opening records, missing/sparse H4
observations, zero-equity drawdown guards, overlapping H4 rows, and inactive-market intervals.
Critical mutation targets cover both trade extraction and portfolio sizing, and Linux run
`30342712156` passes their exact-name ratchet.

## Deferred checks

No required gate is deferred for infrastructure. The rebases change the reviewed code context by
composing issue #99 with PR #96 in the same H4
