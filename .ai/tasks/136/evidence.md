...
HEAD: e576bc4a8505f92bf8a2d6dd81f16cfdfc25bb9f
...
| `check-task-artifact` | PowerShell: `$env:PYTHONUTF8='1'; just --shell "C:\Program Files\Git\bin\sh.exe" check-task-artifact` | 0 | GREEN: automatic discovery finds exactly task 136 and validates 13 AC plus 4 INV |
| `check-pr-evidence` | actual PR #140 body from `gh pr view`, then PowerShell: `$env:PYTHONUTF8='1'; just --shell "C:\Program Files\Git\bin\sh.exe" check-pr-evidence <temporary-body-path>` | 0 | GREEN: PR body is valid, names only `.ai/tasks/136/`, and is backed by current task evidence |
| `pr-ready` | `uv run python -m scripts.quality.pr_ready 136 --base origin/main` | 0 | READY: all 14 required R3 gates pass and the evidence covers code HEAD `e576bc4a8505f92bf8a2d6dd81f16cfdfc25bb9f` |
...
- CI run `30612593696` exposed that automatic task-ID discovery saw both the current
  `.ai/tasks/136/` artifact and a corrected historical task plan. PR #140 now leaves the historical
  file byte-identical to Main and keeps its old executable reference alive through a temporary
  compatibility test. The renamed primary test remains unchanged; the historical one-line
  correction and compatibility removal land together in the immediate follow-up PR.
...
- The historical `ISSUE-111-112` test-name correction is deliberately deferred to the immediate
  follow-up PR, where it will remove the temporary compatibility test in the same change.
