...
|---|---|---|---|
| AC-01 | `tests/test_quality_board.py::test_move_out_of_ready_removes_approved_before_status_change` | RED: focused pre-fix run failed; `move` retained `approved`, then the first fix incorrectly blessed the build-start edge | GREEN: demotions remove first; the `Implementing` parameter is retained as a refusal case |
...
| AC-05 | `tests/test_quality_board.py::test_every_permit_removal_is_verified_by_rereading_state` | RED: `move` did not remove, `start` used a generic error, and `withdraw` did not exist | GREEN: move/start/withdraw refuse a sticky label; three `on_write` tests exercise concurrent status and label changes |
...
| INV-03 | `tests/test_quality_board.py::test_start_guard_condition_set_is_unchanged` | Existing focused cases passed before implementation but did not prove the complete condition set | GREEN: all 224 status/label combinations match the pre-registered Start predicate; ordering remains pinned separately |
...
`uv run pytest -q tests/test_quality_board.py`: **27 failed, 8 passed**. The review-remediation
...
49 passed**. The focused green counts are recorded in `evidence.md`.
