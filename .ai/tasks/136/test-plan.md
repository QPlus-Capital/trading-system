...
| AC-03 | `tests/test_quality_board.py::test_withdraw_removes_approved_without_moving_card` | RED: `BoardService.withdraw` did not exist | GREEN: permit removed with status unchanged, then `start` refuses the unarmed card |
...
| AC-11 | `tests/test_quality_board.py::test_arm_and_approval_write_refusals_name_observed_status` | RED: both messages named only the required status | GREEN: each exact message names the observed status; the lookalike and lost-risk-write tests separately bind exact risk-label reporting |
