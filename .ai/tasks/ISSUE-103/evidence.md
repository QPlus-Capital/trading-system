...
HEAD: 4a24ad698a1883d1be7872a7cb0f7702b67b2433
...
| `check` | `uvx --from rust-just just check` | 0 | Ruff, strict mypy over 180 files, Vulture, and 1,199 tests passed; one Linux-only mutation test was correctly skipped on Windows. |
...
| `adversarial-review` | `uv run python -m scripts.quality.validate_task --task-id ISSUE-103 --base origin/main` | 0 | Twelve counterexamples are recorded with no remaining builder finding; independent Claude live-money review remains required. |
| `invariants` | `uvx --from rust-just just check-invariants` | 0 | 306 critical invariant tests passed, including live risk, account identity, parity, sizing, path risk, regression, and readiness. |
| `mutation-on-touched-critical` | Linux Critical mutation workflow | 1 | **Blocked by infrastructure — Actions quota exhausted until 2026-08-01.** Mutmut requires Linux/fork; no result is claimed, no survivor is classified without measurement, and no baseline or gate was weakened. |
...
| `registry-and-policy` | `uv run pytest -q tests/test_finding_registry.py tests/test_quality_mutation.py` | 0 | Finding F-034 and mutation policy are valid; 19 tests passed and the one Linux-only Mutmut execution test was skipped on Windows. |
...
The focused bridge suite has 25 passing tests, the impact suite has 108, the invariant suite has
306, and the complete deterministic suite has 1,199. Property tests pass twice with the fixed seed.
Coverage includes all five issue-defined runtime boundaries, both legal directions, unknown
integer/string values, and Python's boolean/integer alias.
