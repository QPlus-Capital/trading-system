...
| AC-04 | `test_branch_protection_rejects_future_application_claims` exercises ten future-action forms; the main documentation test requires the application date | RED: all ten review counterexamples passed the four-literal guard | GREEN: all ten are rejected sentence-by-sentence and the applied date remains present |
...
Review regressions additionally prove that a non-matrix `strategy` remains valid, a reusable
workflow job is refused because it changes the emitted context, and a renamed closing section
heading cannot silently widen required-context parsing to the rest of the document.
