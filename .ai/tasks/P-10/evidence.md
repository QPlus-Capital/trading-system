...
HEAD: 3bbeb09f591b005616b50b6e54607efeea5ae0ff
...
| `check` | `uvx --from rust-just just check` | 0 | GREEN: Ruff, strict mypy over 178 files, Vulture, and pytest passed; 1,128 passed and one Linux-only mutation test skipped on Windows |
| `impacted-tests` | `uvx --from rust-just just check-fast origin/main` | 0 | GREEN: 525 direct/transitive tests passed; no unknown/dynamic edge was discovered |
...
| `mutation-on-touched-critical` | Linux Critical mutation workflow run `30224897892` on `3bbeb09f591b005616b50b6e54607efeea5ae0ff` | 0 | GREEN: weakened-test probe and exact ratchet passed; 3,558/3,920 killed, 362 exactly classified survivors, score 0.9077 versus 0.8998 |
...
equivalent P-04 mutants alternated between killed and survived under runner load. Both redundant
zero-low arguments were removed. An independent 10,000-path comparison produced byte-identical
arrays with SHA-256
...
mutants rather than tolerating them. PR run `30224459131` then exposed the real untested
`random == restart_probability` boundary; a deterministic fake-RNG fixture now proves that
equality continues the current block (`<`, not `<=`). Final Linux run `30224897892` passed on its
own merits.
...
Final Linux run `30224897892` passed the weakened-test self-check and exact survivor ratchet:
3,558 killed of 3,920 selected mutants, 362 individually explained survivors, no timeout,
uncovered, suspicious, interrupted, or crashed mutant, and score `0.9077`. P-10 raises the prior
