...
HEAD: 3f336b52f516ce741aad3392d39070fd1d931b7d
...
| `check` | `uvx --from rust-just just check` | 0 | Ruff, strict mypy over 180 files, Vulture, and 1,185 tests passed; one Linux-only mutation test skipped on Windows. |
...
| `adversarial-review` | `uv run python -m scripts.quality.validate_task --task-id ISSUE-60 --base origin/main` | 0 | Three findings and ten counterexamples are recorded; the adjacent metric-moving direction defect is isolated in issue #95. |
| `invariants` | `uvx --from rust-just just check-invariants` | 0 | 308 critical tests passed, including live limits, signal parity, return attribution, risk, scenarios, regression, and readiness. |
...
| `no-autonomous-merge` | `git branch --show-current` and repository merge-policy review | 0 | Feature branch `codex/issue-60-market-trades-swap-columns`; no pull request, merge, or auto-merge action was taken. |
...
The full local suite passes 1,185 tests, the impact suite passes 465, integration passes 59,
invariants pass 308, and properties pass 20 twice. The Linux Critical mutation gate cannot run
locally on Windows and GitHub Actions is quota-blocked. No mutation baseline, survivor allowance,
timeout, score, or threshold was changed or guessed.
...
- No PR is opened in BUILD-ONLY mode.
