...
:mod:`qplus.live.accounts`). The runner refuses to trade unless the connected account's login
...
enabled, and all symbols in Market Watch):
    ...
    uv run python -m qplus.live.run --account mex                 # demo, SIGNAL_ONLY dry-run
    uv run python -m qplus.live.run --account ttp --once          # TTP, a single cycle
    uv run python -m qplus.live.run --account ttp --mode execute  # TTP, place REAL orders
