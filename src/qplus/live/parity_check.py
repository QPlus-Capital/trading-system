...
Run from the repo root with the MT5 terminal open + logged in::
    ...
    uv run python -m qplus.live.parity_check
"""
...
from qplus.live.mt5_bridge import Bar, Mt5Bridge
...
def main() -> None:
    ...
    cfg = load_config_module(repo / "config" / "live" / "paper_rsi_wpr_bb.py")
    ...
    try:
        ...
        for factory, csv, _lev, _sl, _tp in cfg.MARKETS:
            ...
            r = compare(mt5_bars, load_csv_bars(repo / csv), params)
