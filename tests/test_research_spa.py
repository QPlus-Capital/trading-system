...
def test_spa_null_calibration_is_uniform_and_rejects_near_five_percent() -> None:
    ...
    calibrated = np.asarray(p_values)

    rejection_rate = float(np.mean(calibrated <= 0.05))
    assert 0.02 <= rejection_rate <= 0.08
    assert 0.42 <= float(calibrated.mean()) <= 0.58
    q25, q75 = np.quantile(calibrated, [0.25, 0.75])
...
def test_spa_independent_null_calibration_is_not_anti_conservative() -> None:
    p_values = np.asarray(
        [
            spa_test(
                _noise_family(10_000 + index, days=500),
                mean_block_length=5,
                replications=249,
                seed=20_000 + index,
            ).p_value
